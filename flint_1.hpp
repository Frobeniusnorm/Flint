...
template <typename T> struct Tensor<T, 1> {
		 ...
		 * (`storage_type` is a recursive defined type definition, for `n=1` it
		 * is just an alias for `std::vector`). E.g.
		 *
		 * @code{
		 ...
		 * (`init_type` is a recursive defined type definition, for `n=1` it is
		 * just an alias for `std::initializer_list`). E.g.
		 *
		 * @code{
		...
		Tensor<T, 1> reduce_sum() {
			return Tensor<T, 1>(freduce_sum(node, 0), 1);
		}
		...
		Tensor<T, 1> reduce_mul() {
			return Tensor<T, 1>(freduce_mul(node, 0), 1);
		}
		 ...
		 * operation has to duplicate the complete data. Since that is a memory
		 * heavy and slow operation, it is recommended to use the index operator
		 ...
		 * std::cout << (-foo)() << std::endl;
		 * // Tensor<FLOAT32, shape: 4>([3.000000, -3.141592,
		 * -42.079800, 4.300000])
		 * }
		 */
		Tensor<T, 1> operator-() const {
			return Tensor<T, 1>(fneg(node), shape);
		}
		/**
		...
		Tensor<int, 1> sign() const {
			return Tensor<int, 1>(fsign(node), shape);
		}
		/**
		...
		/**
		 * Elementwise minimum of this Tensor and `other`. Per element either
		 * the entry of this Tensor or `other` is returned, depending on which
		...
		/**
		 * Elementwise minimum of this Tensor and the constant `other`. Per
		 * element either the entry of this Tensor or `other` is returned,
		 * depending on which is smaller. The datatype of the result is the
		 * datatype with higher precedence.
		 * */
		template <typename K>
		...
		/**
		 * Elementwise maximum of this Tensor and `other`. Per element either
		 * the entry of this Tensor or `other` is returned, depending on which
		...
		/**
		 * Elementwise maximum of this Tensor and the constant `other`. Per
		 * element either the entry of this Tensor or `other` is returned,
		 * depending on which is larger. The datatype of the result is the
		 * datatype with higher precedence.
		 * */
		template <typename K>
		...
		/**
		 * Slices a selection of the Tensor beginning by `start` (inclusive),
		 * ending with `end` (exclusive) by a step size `step`. The step size
		 * may be negative in which case traversal order changes, therefor
		 * `start > end` must hold (for forward traversal of course `end >
		 * start`). E.g.
		 *
		 * @code{
		 ...
		 * Calls the overloaded one dimensional slice operation with the
		 * corresponding attributes of `r1`.
		 *
		 ...
		 * The window of size `window_size` is slid  along the Tensor starting
		 * from the beginning and moving `step_size` entries. The windows are
		 * concatenated in a extra dimension, which becomes the first dimension
		 * of the result Tensor. E.g.
		 *
		 * @code{
		...
		/**
		 * Removes the gradient mark (ans subsequent memory overhead) for this
