...
struct Flint {
		...
		static Tensor<float, 3> load_image(std::string path) {
			...
			return Tensor<float, 3>(
				node, std::array<size_t, 3>{node->operation.shape[0],
											node->operation.shape[1],
											node->operation.shape[2]});
		}
		/**
		 * Expects an image in `t` with shape width, height, channels (the
		 * number of channels will be passed to stbi). `path` is the filepath to
		 * which the image should be written, `format` denotes the file type of
		 * the output file.
		 */
		 ...
		 * std::array<size_t, 3>(2, 2, 2)); std::cout << foo << std::endl;
		...
		/**
		 * Generates a Tensor containing the single given value in every entry.
		 * The resulting Tensor will have a dimensionality of `sizeof...(args)`
		 * and a shape denoted by each entry in `sizes`. e.g.
		 *
		 * @code{
		...
		static Tensor<T, sizeof...(args)> constant(T value, args... sizes) {
			std::array<size_t, sizeof...(args)> shape{
		 ...
		 * dimension, create multiple such tensors with `arange`. Uses an array
		 ...
		 * dimension `ax` for each element, i.e. each entry is its index in that
		 * corresponding dimension. If you need to index more than one
		 * dimension, create multiple such tensors with `arange`.
		 */
		template <typename... args>
