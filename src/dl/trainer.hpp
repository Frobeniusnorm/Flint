...
class StaticLoader : public DataLoader {
		FGraphNode *train_data;
		FGraphNode *train_images;
		FGraphNode *validate_data;
		FGraphNode *validate_labels;
		FGraphNode *test_data;
		FGraphNode *test_labels;

	public:
		StaticLoader(size_t batch_size, FGraphNode *train_data,
					 FGraphNode *train_images, FGraphNode *validate_data,
					 FGraphNode *validate_labels, FGraphNode *test_data,
					 FGraphNode *test_labels)
			: DataLoader(batch_size), train_data(train_data),
			  train_images(train_images), validate_data(validate_data),
			  validate_labels(validate_labels), test_data(test_data),
		...
		struct IDXFormatLoader : public DataLoader {
				...
				IDXFormatLoader(size_t batch_size,
			...
			private:
				...
				FGraphNode *training_data = nullptr, *validation_data = nullptr,
						   *test_data = nullptr;
				FGraphNode *training_labels = nullptr,
						   *validation_labels = nullptr, *test_labels = nullptr;
		...
		struct Optimizer {
				 ...
				 * Do not set the reference counter as this is done by the
				 * trainer.
				 */
		...
		struct Adam : public Optimizer {
				...
				Adam(float learning_rate, float b1, float b2,
					...
					: epsilon(epsilon), learning_rate(learning_rate), b1(b1),
					  b2(b2) {}
				~Adam() {
					if (m) {
						m->reference_counter--;
				...
				FGraphNode *optimize(FGraphNode *weight,
									 FGraphNode *gradient) override;
		...
		struct TrainingMetrics {
				 ...
				 * `is_epoch` is false, it is the time of the single batch)*/
				double avg_batch_time_ms;
				/** Average time for passing a batch through the model per layer
				 * (if `is_epoch` is false, it is the time of the single batch).
				 * Each layer is given with its name and its execution time.*/
				std::vector<std::pair<std::string, double>>
					avg_batch_time_per_layer_ms;
		};
		struct Trainer {
				 ...
				 * The `DataLoader`, `GraphModel` and `Optimizer` have to be
				 * maintained by whoever passed them and they have to live at
				 * least as long as the `Trainer`. The data for the training and
				 * validation will be taken from the `DataLoader`. The model
				 * `model` will be trained. The `opt` optimizer will be used to
				 * optimize the weights after each batch is passed through the
				 * model. The `loss` Loss function calculates the loss between
				 * the output of the model and the expected output from the
				 * labeled dataset.
				 */
				 ...
				 * The `DataLoader` has to be maintained by whoever passed it
				 * and it has to live at least as long as the trainer. The data
				 * for the training and validation will be taken from the
				 * `DataLoader`.
				 */
				void set_data_loader(DataLoader *dl) { this->data = dl; }
				/**
				 * The `Optimizer` has to be maintained by whoever passed it and
				 * it has to live at least as long as the trainer. It will be
				 ...
				 * and it has to live at least as long as the trainer. It will
				 ...
				 * (It is split into `batch_size` sized slices in the first
				 * dimension of the input data and each batch has to be passed
				 * through the model once per epoch). The weights of the model
				 * are optimized after each batch. Once all batches have been
				 * run for a epoch, the validation data is passed through the
				 * model and the error reported.
				 *
				 * Make sure the `DataLoader` and `Model` are valid for the call
				 * of this function.
				 */
