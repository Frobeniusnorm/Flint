...
static FGraphNode *local_gradient(FGraphNode *y, int dx_i,
	...
	case FCONCAT: {
		...
		if (0 == dx_i) {
			...
			for (int i = 0; i < b->operation.dimensions; i++) {
				...
				stop[i] = ax == i
							  ? a->operation.shape[i] + b->operation.shape[i]
							  : b->operation.shape[i];
			}
			return fslice(prev_adj, start.data(), stop.data());
		} else
	...
	case FSLIDING_WINDOW: {
		...
		if (0 == dx_i) {
			...
			FGraphNode *res = funslide_window(prev_adj, a->operation.shape,
											  sliding_win->step);
			return res;
		} else
			return nullptr;
			...
			if (a->operation.dimensions != kernel->operation.dimensions) {
				...
				for (int i = 0; i < kernel->operation.shape[0]; i++) {
					...
					FGraphNode *sk = fflatten_dimension(
						fslice(kernel, start_kernel.data(), end_kernel.data()),
						1);
	...
	case FGRADIENT_CONVOLVE1: {
		...
		if (1 == dx_i) {
			...
			if (!op.shape)
				return nullptr;
			memcpy(op.shape, a->operation.shape,
	...
	case FGRADIENT_CONVOLVE2: {
		...
		const unsigned int *steps =
			(unsigned int *)y->operation.additional_data;
		const bool multifilter =
			a->operation.dimensions != b->operation.dimensions;
		if (0 == dx_i) {
	...
	case FREDUCE_SUM: {
		...
		if (a == dx) {
			...
			return frepeat(freshape(prev_adj, ns.data(), ns.size()),
						   rep.data());
		} else
			return nullptr;
	}
	case FREDUCE_MUL: {
		...
		if (a == dx) {
			...
			// broadcast y
			FGraphNode *ls =
				frepeat(freshape(y, ns.data(), ns.size()), rep.data());
	...
	case FSLICE: {
		...
		for (int i = 0; i < a->operation.dimensions; i++) {
			start[i] =
				slice->step[i] >= 0 ? slice->start[i] : slice->end[i] + 1;
		}
	...
	case FEXTEND: {
		...
		for (int i = 0; i < a->operation.dimensions; i++) {
			...
			ends[i] =
				a->operation.shape[i] * extend->step[i] + extend->start[i];
