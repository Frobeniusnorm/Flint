...
func CreateGraphDataInt(data []Int, shape Shape) (GraphNode, error) {
	if uint(len(data)) != shape.NumElements() {
		return GraphNode{}, buildErrorCustom(ErrIllegalDimensionality, "data and shape mismatch")
	}
	newShape := fromGoArrayToC[uint, C.size_t](shape)
	newData := fromGoArrayToC[Int, C.int](data)
...
func CreateGraphDataLong(data []Long, shape Shape) (GraphNode, error) {
	...
	newShape := fromGoArrayToC[uint, C.size_t](shape)
	newData := fromGoArrayToC[Long, C.long](data)
...
func CreateGraphDataFloat(data []Float, shape Shape) (GraphNode, error) {
	...
	newShape := fromGoArrayToC[uint, C.size_t](shape)
	newData := fromGoArrayToC[Float, C.float](data)
...
func CreateGraphDataDouble(data []Double, shape Shape) (GraphNode, error) {
	...
	newShape := fromGoArrayToC[uint, C.size_t](shape)
	newData := fromGoArrayToC[Double, C.double](data)
