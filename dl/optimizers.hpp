...
template <int n> struct Optimizer {
  ...
  /**
   * Takes the old weight and its gradient to the error tensor and updates it,
   * i.e. returns the updated version of the weight.
   */
...
template <int n> struct Adam : public Optimizer<n> {
   ...
   *     optimization, i.e. the proportion weights are updated. Higher values
   *     (e.g. 0.2) lead to a faster convergence, while lower values yield more
...
struct AdamFactory {
   ...
   *  - `b2`: (sometimes called `beta2`) the exponential decay rate for the
   *     second moment estimates.
   * All Adam instances generated by `generate_optimizer` are constructed with
