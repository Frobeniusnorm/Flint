...
//  operations
/** Serializes this node and returns a array of chars in which the serialized
 * data will be written. The returned array is allocated on the systems heap
 * with `malloc`, so you have to free it after you are done with it. The number
 * of bytes that the returned array has is written into the memory
 * `bytes_written` points to.
 * If the node doesn't have result data, it is executed first.
 */
